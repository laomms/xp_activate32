...
static const ui64 sig[6] = { 0, 0x21840136C85381ULL, 0x44197B83892AD0ULL, 0x1400606322B3B04ULL, 0x1400606322B3B04ULL, 1 };
...
static void sha1_single_block(unsigned char input[64], unsigned char output[20])   //0f 76 86 09 f9 49 4a 2e 85 6a c8 5e d4 80   //29 5d 9e 8f fd ba e0 00 94 6a c8 5e d4 80
{
	unsigned a, b, c, d, e;
	a = 0x67452301;
	b = 0xEFCDAB89;
...
std::wstring splitString(const std::wstring& input, int groupSize, wchar_t delimiter) {
	std::wstring result;
	...
	for (int i = 0; i < groups; i++) {
		result += input.substr(i * groupSize, groupSize);
		result += delimiter;
	}
...
static void OnActivationIdChange(HWND hDlg)
{
	...
	std::wstring wstr_id(installation_id);
	if (wstr_id.empty()) {
		std::wcout << L"installation_id为空值！" << std::endl;
		return ;
	}
